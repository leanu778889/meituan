...
//    removeTimeout   : 11//提示框消失时间
    ...
    onUploadSuccess	: function(file, data, response) {
        ...
        var html="";
        html+="<li><div class='delUploadFile'></div>";
        ...
        if(this.settings.showalt){
            html+="<div class='upload_title'><input type='text' name='"+upload_file_id.substr(13)+"["+_index+"][alt]' value='图片描述' onfocus=\"if(this.value=='图片描述')this.value=''\"/></div>";
        }
...
function alter_upload_msg(obj){
    ...
    var msg_obj = $("."+upload_file_id+"_msg");
    if(allowUploadNums>0){
        ...
        msg_obj.html("你已经上传"+uploadsSuccessful+"个文件,还可以上传"+allowUploadNums+"个文件，每个文件不能超过"+file_size_limit);
