...
class Compile
{
    ...
    private function loadParseTags()
    {
        ...
        if (!empty($tags) && is_array($tags)) { //如果配置文件中存在标签定义
            foreach ($tags as $k) { //加载其他模块或应用中的标签库
                ...
                if (class_exists($arr[$count - 1], false)) {
                    continue;
                }
                if ($count > 1) {
                    switch (count($arr)) {
                        case 2:
                            $tagFile = GROUP_PATH . $arr[0] . '/Lib/' . $arr[1] . '.class.php';
                            break;
                        case 3:
                            $tagFile = GROUP_PATH . $arr[0] . '/' . $arr[1] . '/Lib/' . $arr[1] . '.Tool.php';
                            break;
                    }
    ...
    private function compileTag($tagName, &$tagObj, $block = 1)
    {
        ...
        foreach ($arr as $k) {
            ...
            $content = call_user_func_array(array($tagObj, '_' . $tagName), array($attr, $k[2], $this));
...
//        }
    ...
    {
        $preg = '/\$Hd.(get|post|request|cookie|Session)\./ise';
