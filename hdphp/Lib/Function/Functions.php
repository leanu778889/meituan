...
function C($name = null, $value = null)
{
    ...
    if (is_string($name)) {
        ...
        if (!strstr($name, '.')) {
            ...
            //加载语言包
            if($name=='language'){
                is_file(COMMON_LANGUAGE_PATH . $value.'.php') and L(require COMMON_LANGUAGE_PATH . $value.'.php');
                //加载应用语言包
                is_file(LANGUAGE_PATH . $value.'.php') and L(require LANGUAGE_PATH . $value.'.php');
            }
